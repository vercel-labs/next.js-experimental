...
A cache directive gives a result a lifetime, which Next.js uses for time-based revalidation and rendering optimizations. For on-demand revalidation after a mutation, see [Revalidating](/docs/app/getting-started/revalidating). See [Prerendering](#prerendering) for how cached results become part of the static shell and may be included in a [prefetch](#prefetching).
...
Browsers and DOM-capable crawlers such as `Googlebot` receive the static shell instantly. HTML-limited bots, which cannot execute JavaScript and need metadata in the `<head>` of a complete document, are detected by their user agent and handled differently: Next.js skips the shell and renders the entire page dynamically at request time, then sends the finished HTML once the render completes. You can control which user agents are treated this way with the [`htmlLimitedBots`](/docs/app/api-reference/config/next-config-js/htmlLimitedBots) config option.

Because the shell is re-rendered instead of reused, work that completed during prerendering now runs at request time for these bots, and the response is not served from the route cache. If part of your shell depends on inputs that only exist while prerendering, such as build-time data or values that are not reachable in the request-time environment, a page that loads for a person can fail to render for an HTML-limited bot. Make sure the data your shell relies on is also available at request time. See [Bots and crawlers](/docs/app/guides/streaming#bots-and-crawlers) in the Streaming guide for more details.
